...
class Bhp_pemusnahan_model extends CI_Model {
    ...
    function nourut($kode_pengadaan){
        ...
        $q = $this->db->query("select MAX(RIGHT(id_inv_inventaris_habispakai_opname,6)) as kd_max from inv_inventaris_habispakai_opname where (LEFT(id_inv_inventaris_habispakai_opname,$jmlkode))=".'"'.$kode_pengadaan.'"'."");
        $nourut="";
        if($q->num_rows()>0)
        {
