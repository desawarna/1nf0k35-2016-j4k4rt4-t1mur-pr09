...
class Bhp_distribusi_model extends CI_Model {
   ...
   function insert_entry()
    ...
    function nourut($kode_pengadaan){
        $jmlkode = strlen($kode_pengadaan);
        $q = $this->db->query("select MAX(RIGHT(id_inv_inventaris_habispakai_distribusi,6)) as kd_max from inv_inventaris_habispakai_distribusi where (LEFT(id_inv_inventaris_habispakai_distribusi,$jmlkode))=$kode_pengadaan");
        $nourut="";
        if($q->num_rows()>0)
        {
            foreach($q->result() as $k)
